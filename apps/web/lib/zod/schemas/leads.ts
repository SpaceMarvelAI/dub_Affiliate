...
export const trackLeadRequestSchema = z.object({
  ...
  mode: z
    ...
    .default("async")
    .describe(
      "The mode to use for tracking the lead event. `async` will not block the request; `wait` will block the request until the lead event is fully recorded in Space Marvel; `deferred` will defer the lead event creation to a subsequent request.",
    ),
  eventQuantity: z
    .number()
    .int()
